...
function selectIdea(newIdeaId, doNotOpen) {
  ...
  openstadSetCookie('eventId' + voteBlockIdentifier, eventId);
  voteCreatorElement.querySelector('input[name=eventId]').value = eventId;
  ...
  var eventContainer = document.querySelector('#event-' + eventId);
  var imageContainer = eventContainer ? eventContainer.querySelector('.image') : false;
  ...
  var eventPresentOnPage = imageContainer ? true : false;


  if (!imageHtml) {
    imageUrl = localStorage.getItem('eventImageUrl'+ eventId);
  ...
  if (imageUrl) {
    localStorage.setItem('eventImageUrl'+ eventId, imageUrl);
  }
  ...
  var desciptionContainer = eventContainer ? eventContainer.querySelector('.voteblock-description'): false;
  ...
  //add event-title
  var eventTitle = $('#event-' + eventId + ' .title').first().text();
  if (previewDescriptionHTML) {
    $('.selected-event-title').html(previewDescriptionHTML);
  } else {
    $('.selected-event-title').text(eventTitle);
  }

  previewElement.setAttribute('title', 'Ontwerp "' + eventTitle + '" gekozen. Druk op enter om deze te verwijderen.');


  if (doShowImage && eventPresentOnPage) {
    doShowImage(eventId, previewElement);
  }

  setNextButton();
  setLoginUrlWithIdeaId(eventId);
...
function setLoginUrlWithIdeaId(eventId) {
  var $form = $('#auth-form');

  if ($form.length > 0) {
    var action = $form.attr('action');
    $form.attr('action', action + encodeURIComponent('?' + returnFromKey + '=' + eventId))
  }

  var $validateAuthButton =  $('.validate-auth-button');

  if ($validateAuthButton.length > 0) {
    var url =  window.siteUrl + loginUrl + '?' + returnToKey + '=' + currentPathname + '?' + returnFromKey + '=' + eventId;
...
function unSelectIdea(event) {
  ...
  eventId = undefined;
  openstadEraseCookie('eventId' + voteBlockIdentifier);
  voteCreatorElement.querySelector('input[name=eventId]').value = '';
...
function eventOverviewClickPreview(event) {
...
function eventOverviewKeyDownPreview(event) {
  if (event.keyCode == 13) {
    if (eventId) {
      unSelectIdea(event);
    } else {
...
function overviewScrollToIdeas() {
  var elem = document.getElementById('events-anchor');
...
function sendVote() {
  ...
  var eventId = document.querySelector('input[name=eventId]').value;
...
//  }
  ...
  $.ajax({
    method: 'POST',
  //  url: "/api/site/"+siteId+"/event/" + eventId + "/vote",
    url: window.siteUrl + "/vote",
    data: { opinion: 'yes', eventId: eventId },
    json: true,
...
function showVoteCreator() {
  ...
  if (eventId && doUpdateIdea) selectIdea(eventId, true);
}
...
//var eventId = match ? match[1] : undefined;
