...
module.exports = {
    ...
    label: 'Event overview (copy from Resources)',
    addFields: fields,
    playerData: false,
    construct: function (self, options) {
        ...
        self.load = function (req, widgets, next) {
            ...
            widgets.forEach((widget) => {
                ...
                widget.renderString = (data, activeEvent) => {
                    data.activeEvent = activeEvent ? activeEvent : {};
        ...
        self.formatWidgetResponse = (widget, response, queryParams, pathname) => {
            ...
            widget.activeEvents = response.records ? response.records.map((record) => {
            ...
            widget.allActiveEvents = widget.allActiveEvents || {};
            
            widget.allActiveEvents[widget.resource] = response.records ? response.records.map((record) => {
              let daysOld = parseInt( ( Date.now() - new Date(record.startDate).getTime() ) / ( 24 * 60 * 60 * 1000 ) );
              record.countdown = widget.countdownPeriod - daysOld;
              return record;
            }) : [];
        ...
        self.output = function (widget, options) {
            ...
            if (widget.filterResources) {
                ...
                widget.activeEvents = resourceIds.length > 0 ? widget.activeEvents.filter(idea => resourceIds.indexOf(idea.id) !== -1) : widget.activeEvents;
            }
